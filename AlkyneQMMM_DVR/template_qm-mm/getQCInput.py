...
mm_cutoff = 2.8 #mm cutoff distance in nanometers
...
def getQCInput(geometrySymbols, geom_pt, frame, functional, basis_set):
    ...
    #Load implicit charge environment
    qEns, xEns, yEns, zEns = np.loadtxt(framePath + "mmregion_frame"+str(frame)+".txt", delimiter=' ', unpack = True)

    external_charges = ""
    for j in range(len(qEns)):
         external_charges += "\n{:.5f} {:.5f} {:.5f} {:.5f}".format(xEns[j],yEns[j],zEns[j],qEns[j])

    external_chargesSection = CustomSection(title = "external_charges",
                                            keywords = {external_charges : ""})
    ...
    qc_input = QchemInput(molecule,
                      ...
                      extra_sections=[external_chargesSection])
