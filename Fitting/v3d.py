#Hacked by Clyde in 08/2023 to remove extra dependancies
#and otherwise be more useful for MD parameter optimization
...
phi_lim = 0.017 #op.Params.v3d_tors_angle_lim
tors_cos_tol = 1e-10 #op.Params.v3d_tors_cos_tol
...
def oofp(A, B, C, D):
    """
    Compute and return angle in Out-OF-Plane angle in radians A-B-C-d
    returns false if bond angles are too large for good Out-OF-Plane angle definition
    ...
    if sin(phi_CBD) < tors_cos_tol:  # reusing parameter
        print(f"Angle: {C}, {B}, {D} is to close to zero in oofp\n")
